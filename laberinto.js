...
function getShortestPath() {
    ...
    const end = [
        LEVELS[currentLevelIndex].cols - 2,
        LEVELS[currentLevelIndex].rows - 2
    ];
    ...
    while (queue.length > 0) {
        ...
        for (const [dx, dy] of dirs) {
            ...
            if (
                ny > 0 &&
                ny < LEVELS[currentLevelIndex].rows &&
                nx > 0 &&
                nx < LEVELS[currentLevelIndex].cols &&
                maze[ny][nx] !== 1 &&
                !visited.has(key)
            ) {
...
function startLevel(levelIndex) {
    ...
    canvas.width =
        config.cols * config.tileSize;

    canvas.height =
        config.rows * config.tileSize;
